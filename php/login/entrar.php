...
if (esta_logado()) {
    if (in_array($papel, $adminRoles, true)) {
        redirecionar('/TCC-etec/php/login/adms/logado/index.php');
    }
    if (in_array($papel, $secretariaRoles, true)) {
        redirecionar('/TCC-etec/php/secretaria/secretaria.php');
    }
    if (in_array($papel, $alunoRoles, true)) {
        redirecionar('/TCC-etec/php/sou_aluno/index.php');
    }
}

$perfil = strtolower((string)($_GET['perfil'] ?? ''));
if (in_array($perfil, ['adm', 'administrador', 'admin', 'professor', 'prof'], true)) {
    redirecionar('/TCC-etec/php/login/adms/login.html');
}
...
redirecionar('/TCC-etec/php/login/alunos/login.html');
